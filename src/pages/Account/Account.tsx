...
const Account: React.FC = () => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    lname: Yup.string()
      ...
      .required('Required'),
    password: Yup.string()
      .matches(/^[A-Z][a-z0-9]{3,8}$/, 'Password must start with an uppercase letter and be 4-9 characters')
  ...
  return (
    <>
      <Box
        role="presentation"
        sx={{ display: 'flex', justifyContent: 'space-between', margin: '5rem', flexDirection: {sm:'column', md:'row'}, alignItems:{sm:'center', md:'flex-start'}, gap:'1rem' }}
      >
        ...
        <Typography>Welcome! <span className='text-color'>{`${formData.fname} ${formData.lname}`}</span></Typography>
      </Box>

      <Box sx={{ display: 'flex', justifyContent: 'space-around', flexDirection: {sm:'column', md:'row'}, alignItems:{sm:'center', md:'flex-start'}, gap:'1rem' }}>
        {/* Sidebar Navigation */}
        ...
        {/* Form Section */}
        <Box sx={{ p: 4, boxShadow: 2, width: '100%', maxWidth: 500, marginBottom:'2rem' }}>
          <Typography sx={{ fontWeight: 'bold', mb: 2 }} className='text-color'>Edit Your Profile</Typography>
          <form onSubmit={formik.handleSubmit}>
            ...
            <FormInput name="confirmPassword" label="Confirm Password" type="password" formik={formik} />
           <Box sx={{display:'flex', justifyContent:'flex-end'}}>
             <Button
              variant="contained"
              sx={{ mt: 2 , backgroundColor:'transparent', color:'black', mr:2}}
            >
              Cancel
            </Button>
            <Button
